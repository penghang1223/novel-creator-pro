...
3. **长篇续写**：自动执行章节前检查（9个问题）→ 记忆唤醒 → 生成场景写作卡 → **读取写作速查卡 → 提取本章约束**（节奏模板/对话要素/爽点密度/去AI规则）→ **Pass 1 剧情稿**（不管AI词，截断点停笔）→ **Pass 2 AI词清理**（只改词汇，不动叙事，参照速查卡"每段三刀"法则+20条硬性禁止）→ 写后审计（逻辑→AI词→对话）→ 风格校准 → 人物一致性 → 记忆回填
