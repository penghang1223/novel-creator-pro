...
def validate_file(path: str, label: str = "文件") -> None:
    """验证文件路径存在且可读"""
    if not os.path.exists(path):
...
def validate_json_file(path: str, label: str = "JSON文件") -> Dict[str, Any]:
    """验证JSON文件存在且格式正确，返回解析后的内容"""
    validate_file(path, label)
    try:
        with open(path, "r", encoding="utf-8-sig") as handle:
            return json.load(handle)
    except json.JSONDecodeError as e:
        raise ValueError(f"{label}JSON格式错误: {path} - {e}")
...
def main() -> None:
    ...
    if args.command == "create":
        try:
            ...
            print(f"错误: JSON解析失败: {e}", file=__import__("sys").stderr)
            return
        ...
        if not os.path.exists(args.input):
            print(f"错误: 项目配置文件不存在: {args.input}", file=__import__("sys").stderr)
            return
        result = manager.bootstrap_project(args.input)
        print_json(result)
    elif args.command == "chapter-pack":
        if args.chapter <= 0:
            print("错误: 章节号必须为正整数", file=__import__("sys").stderr)
            return
        ...
        if not os.path.exists(args.input):
            print(f"错误: 章节摘要文件不存在: {args.input}", file=__import__("sys").stderr)
            return
