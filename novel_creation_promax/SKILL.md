...
**Step C：读取出场人物档案**
从 `设定/人物设定.md`（或 `设定/人物档案/`）提取本章出场角色的 9 个关键字段：

| 字段 | 来源维度 | 用途 |
|------|---------|------|
| 矛盾特质 | §一 核心性格 | 防止角色扁平化 |
| 行为指纹 | §二 | 动作唯一化，形成辨识度 |
| 对话声纹 | §三 口头禅/句式/禁用词 | 对话遮住名字仍可辨认 |
| 动机优先级 | §四 | 每个行动必须服务目标 |
| 决策模型 | §五 风险偏好/撒谎/留后手 | 驱动行为选择 |
| 反差开关 | §八 表层/真实/触发 | 控制反差揭露节奏 |
| 情绪触发器 | §七 正向/负向/禁区 | 每场冲突至少触发一个按钮 |
| 渴望/恐惧 | §一 | 核心驱动力，行为底层逻辑 |
| 行为红线 | §四 禁止行为 | 硬约束，违规即 OOC |
...
- 角色B：矛盾=xxx | 声纹=xxx | 优先级=xxx | 反差=xxx | 触发器=xxx | 红线=xxx
