...
**约束必须在写中分段生效，不是每段自检，不是写后门禁。**

#### 写中分段检查（每300-500字自检一次）

写作时不要逐段打断节奏，改为：
- 每写 300-500 字 → 做一次小检查（无聊/重复/推进）
- AI味检测不放在写中（会毁节奏），统一移到写后处理

写中只保留3个实时感知：
1. **有没有无聊？** — 这段读者会跳过去吗？
2. **有没有重复？** — 和前一段在重复说同一件事吗？
3. **有没有推进？** — 情节/情绪/关系有往前走吗？

每次300-500字自检时，必须额外检查：**当前累计字数 / 目标3000字的进度**。如果最后一个场景写完了总字数仍低于2500，必须在后续场景中补足，不能等写后审计再补。

**写中感知清单（每300-500字检查一次）**

检查以下问题，发现问当时就改，不是写完再改：
...
### 硬门禁 0.5：章节驱动引擎（写前驱动核心）

**来源**：2026-05-01 流程审计。根因：约束很多但缺主线驱动，写的时候"正确"但不一定"好看"。

**规则**：每章正文写作前，**必须先回答以下3个问题**，没这3个不准进入9问系统：
...
| **场景字数预算** | 平台要求(2800-3200) | 本章拆分4-6个场景，每个场景分配字数，总计目标3000字 |
...
**规则**：以下3种场景**必须先写动作流大纲，再写正文**：

1. **信息博弈** — 双方知道的信息不同，有人在隐瞒/试探/套话
2. **情绪对抗** — 双方有明确的目标冲突或利益对立
3. **隐藏目的** — 角色表面在聊A，实际在推进B（谈判/试探/威胁/说服）

普通日常对话、单向信息传递、角色独白 → 不需要动作流大纲
...
**来源**：2026-05-02 系统重构。根因：清AI词和保剧情连贯冲突——清理时把叙事逻辑和对话风格也清坏了。

**优先级原则**：剧情逻辑 > 角色一致 > AI词清理
...
**规则**：
- 写的时候**完全忽略AI词**（"像是"、"不禁"等词汇不检查）
- 专注于：剧情推进、角色行为逻辑、章节边界、对话信息量
- 每300-500字自检只问三个问题：有没有无聊？有没有重复？有没有推进？
- **章节边界硬约束**：严格在场景写作卡截断点停笔，不蹭到下一章内容
...
- 对话修改必须保证：角色说话方式不变（不改变说话风格）
- **如果某个词替换会导致语义不通，跳过该替换**
- 每个替换必须验证：替换后句子逻辑仍然通顺
...
```
1. 先跑逻辑检查 → 通过后再进入清理
2. 再跑AI词清理 → 逐句检查，优先保留语义
3. 最后跑对话质量检查
```

**如果逻辑检查不通过，不进入AI词清理步骤。**

详见：`references/orchestrator.md` §正文创作执行链
...
- **对话比例 <25%** → 停止写作，增加角色对话
- **黑名单禁用词出现** → 以剧情连贯性为优先，合适则替换，不合适则保留
- **"像"比喻 >1 次/章** → 删除多余比喻
...
- **心声格式**：角色内心独白（心声）必须用叙事格式（`心里在转——内容`），不得加引号（`"内容"`）。加引号的心声会被审计计为对话行，导致乒乓球超标
...
- 本章主爽点有没有兑现？
- 核心冲突有没有展开？
- 结尾有没有钩子？（断章技巧：悬念最高处/打脸前一刻/新冲突出现/情感高潮/身份reveal前。禁止在日常描写/平淡对话/环境描写/心理活动后断章。卡章公式：冲突升级→高潮前一刻断章→下章释放）
...
底线指标（任意一项不达标 → 本章作废）：
- AI词黑名单：黑名单禁用词出现即替换；**不要机械清零**，以剧情连贯性和自然叙事为优先
...
### 硬门禁 5：对话干涩 + 人设冲突检测（写后扫描版）

**这条规则是2026-05-01调整的。位置：在结构检查通过后、AI味扫描前执行。**

结构检查不通过 → 直接重写，不进入此步。通过后对照以下清单快速检查：
